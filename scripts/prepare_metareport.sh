...
cp $CURRENT_PATH"/runs/"$MODEL_NAME"/OMIM_DO/splitpaper/results/llm_pmID_profiles.txt" $METAREPORT_RESULTS_PATH/do_papers_llm_pmID_profiles.txt
cp $CURRENT_PATH"/runs/"$MODEL_NAME"/OMIM_DO/splitabstract/results/llm_pmID_profiles.txt" $METAREPORT_RESULTS_PATH/do_abstracts_llm_pmID_profiles.txt
cp $CURRENT_PATH"/runs/"$MODEL_NAME"/OMIM_ehrhart/splitpaper/results/llm_pmID_profiles.txt" $METAREPORT_RESULTS_PATH/ehrhart_papers_llm_pmID_profiles.txt
cp $CURRENT_PATH"/runs/"$MODEL_NAME"/OMIM_ehrhart/splitabstract/results/llm_pmID_profiles.txt" $METAREPORT_RESULTS_PATH/ehrhart_abstracts_llm_pmID_profiles.txt
...
transpose_table -i $META_GS_COUNTS_FOLD_JOINED/phenotype_counts | awk 'BEGIN{OFS="\t"}{print $1,$4,$2,$3,$7,$5,$6}' > $META_GS_COUNTS_FOLD_JOINED/phenotype_counts_transposed
...
transpose_table -i $META_GS_COUNTS_FOLD_JOINED/disease_counts | awk 'BEGIN{OFS="\t"}{print $1,$4,$2,$3,$7,$5,$6}' > $META_GS_COUNTS_FOLD_JOINED/disease_counts_transposed
